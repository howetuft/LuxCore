...
template<typename T> using ArrayOfOptionals =
	std::array<std::optional<std::vector<T>>, EXTMESH_MAX_DATA_COUNT>;
...
public:
	ExtTriangleMesh(
		...
		std::optional<std::vector<Normal>> meshNormals = std::nullopt,
		std::optional<std::vector<UV>> meshUVs = std::nullopt,
		std::optional<std::vector<Spectrum>> meshCols = std::nullopt,
		std::optional<std::vector<float>> meshAlphas = std::nullopt,
	...
	ExtTriangleMesh(
		...
		std::vector<Point> meshVertices,
		std::vector<Triangle> meshTris,
		std::optional<std::vector<Normal>> meshNormals,
	...
	void SetVertexAOV(const u_int dataIndex, std::optional<std::vector<float>> values) {
	...
	void SetTriAOV(const u_int dataIndex, std::optional<std::vector<float>> values) {
	...
	void SetUVs(const u_int dataIndex, std::vector<UV> data) {
		uvs[dataIndex] = data;
	}
	...
	void SetColors(const u_int dataIndex, std::vector<Spectrum> data) {
		cols[dataIndex] = data;
	}
	...
	void SetAlphas(const u_int dataIndex, std::vector<float> data) {
	...
	std::optional<std::vector<Normal>> ComputeNormals();
	...
	ExtTriangleMeshPtr CopyExt(
			std::optional<std::vector<Point>> meshVertices,
			std::optional<std::vector<Triangle>> meshTris,
			std::optional<std::vector<Normal>> meshNormals,
			std::optional<ArrayOfOptionals<UV>> meshUVs,
			std::optional<ArrayOfOptionals<Spectrum>> meshCols,
			std::optional<ArrayOfOptionals<float>> meshAlphas,
	...
	ExtTriangleMeshPtr Copy(
			...
			std::optional<std::vector<Triangle>> meshTris,
			std::optional<std::vector<Normal>> meshNormals,
			std::optional<std::vector<UV>> mUVs,
			std::optional<std::vector<Spectrum>> mCols,
			std::optional<std::vector<float>> mAlphas,
			const float bRadius = 0.f
	) const;
...
public:
	...
	template<class Archive>	void load(Archive &ar, const unsigned int version) {
		...
		ar & hasNormals;
		if (hasNormals) {
			normals = std::make_optional<std::vector<Normal>>(vertCount);
			for (u_int i = 0; i < vertCount; ++i)
				ar & normals.value()[i];
		} else
			normals = std::nullopt;
		triNormals = std::vector<Normal>(triCount);
		...
		for (u_int i = 0; i < EXTMESH_MAX_DATA_COUNT; i++) {
			...
			if (hasUVs) {
				// TODO
				uvs[i] = std::make_optional<std::vector<UV>>(vertCount);
			...
			ar & hasColors;
			if (hasColors) {
				cols[i] = std::make_optional<std::vector<Spectrum>>(vertCount);
			...
			ar & hasAlphas;
			if (hasAlphas) {
				alphas[i] = std::make_optional<std::vector<float>>(vertCount);
			...
			ar & hasVertexAOV;
			if (hasVertexAOV) {
				vertAOV[i] = std::make_optional<std::vector<float>>(vertCount);
			...
			ar & hasTriangleAOV;
			if (hasTriangleAOV) {
				triAOV[i] = std::make_optional<std::vector<float>>(triCount);
	...
	std::vector<Normal> triNormals; // Triangle normals
	std::optional<std::vector<Normal>> normals; // Vertices normals
