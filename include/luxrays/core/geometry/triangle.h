...
public:
	...
	BBox WorldBound(const Buffer<Point>& verts) const {
	...
	bool Intersect(const Ray &ray, const Buffer<Point>& verts, float *t, float *b1, float *b2) const {
		const Point &p0 = verts[v[0]];
		const Point &p1 = verts[v[1]];
		const Point &p2 = verts[v[2]];
	...
	float Area(const Buffer<Point>& verts) const {
		const Point & p0 = verts[v[0]];
		const Point & p1 = verts[v[1]];
		const Point & p2 = verts[v[2]];
	...
	Normal GetGeometryNormal(const Buffer<Point>& verts) const {
		const Point &p0 = verts[v[0]];
		const Point &p1 = verts[v[1]];
		const Point &p2 = verts[v[2]];
	...
	bool GetBaryCoords(const Buffer<Point>& verts, const Point &hitPoint, float *b1, float *b2) const {
		const Point &p0 = verts[v[0]];
		const Point &p1 = verts[v[1]];
		const Point &p2 = verts[v[2]];
