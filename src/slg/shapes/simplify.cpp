...
public:
	...
	ExtTriangleMeshPtr GetExtMesh() const {
		...
		if (hasNormals) {
			newNorms.emplace(vertCount);
			for (u_int i = 0; i < vertCount; ++i)
				(*newNorms)[i] = vertices[i].norm;
		}

		std::optional<std::vector<UV>> newUVs;;
		if (hasUVs) {
		...
		std::optional<std::vector<Spectrum>> newCols;
		if (hasColors) {
		...
		std::optional<std::vector<float>> newAlphas;
		if (hasAlphas) {
		...
		return std::make_shared<ExtTriangleMesh>(
			vertCount, triCount, newVertices, newTris, newNorms,
