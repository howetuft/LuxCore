...
DisplacementShape::DisplacementShape(luxrays::ExtTriangleMeshPtr srcMesh, const Texture &dispMap,
	...
	Optionals<Point> newVertices;
	newVertices.resize(vertCount);
	...
	mesh = srcMesh->Copy(std::move(newVertices), std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt);
