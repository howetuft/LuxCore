...
// Z
Grid ComputeGrid(const std::vector<Point> & points, u_int numPoints) {
	...
	auto boundingbox = parallel_reduce(
		// Range
		blocked_range<const_iter_point>(points.cbegin(), points.cend(), grain),
		...
		// Body:w
		//
		[](const blocked_range<const_iter_point>& r, sixfloats init) -> sixfloats {
...
Partition AssignPointsToGrid(
	const Grid& grid, const std::vector<Point> & points, u_int numPoints
) {
...
//
ClusterMap mergePoints(const std::vector<Point>& points, u_int numPoints, u_int tolerance) {
...
luxrays::ExtTriangleMeshPtr RecreateMesh(
	...
	// Points
	auto newPoints = luxrays::ExtTriangleMesh::AllocVerticesBuffer(numNewPoints);

	// Normals
	std::optional<std::vector<luxrays::Normal>> newNormals;
	const auto srcNormals = srcMesh.GetNormals();
	if (srcMesh.HasNormals()) {
		newNormals = std::vector<luxrays::Normal>(numNewPoints);
	}
	...
	for (u_int i = 0; i < EXTMESH_MAX_DATA_COUNT; ++i) {
		if (srcMesh.HasUVs(i)) {
			newUVs[i] = std::vector<luxrays::UV>(numNewPoints);
		}
	}
	...
	for (u_int i = 0; i < EXTMESH_MAX_DATA_COUNT; ++i) {
		if (srcMesh.HasColors(i)) {
			newColors[i] = std::vector<luxrays::Spectrum>(numNewPoints);
		}
	}
	...
	for (u_int i = 0; i < EXTMESH_MAX_DATA_COUNT; ++i) {
		if (srcMesh.HasAlphas(i)) {
			newAlphas[i] = std::vector<float>(numNewPoints);
		}
	}
	...
	for (u_int i = 0; i < EXTMESH_MAX_DATA_COUNT; ++i) {
		if (srcMesh.HasVertexAOV(i)) {
			newVertexAOVs[i] = std::vector<float>(numNewPoints);
		}
	}
	...
	auto newTriangles = luxrays::ExtTriangleMesh::AllocTrianglesBuffer(numTriangles);
	tbb::parallel_for(
	...
	for (u_int i = 0; i < EXTMESH_MAX_DATA_COUNT; ++i) {
		if (srcMesh.HasVertexAOV(i)) {
			newMesh->SetVertexAOV(i, newVertexAOVs[i]);
		}
	}
