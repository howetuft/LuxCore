...
void Scene::ParseVolumes(const Properties &props) {
	...
	for(const string &key: matKeys) {
		...
		if (matDefs.IsMaterialDefined(volName)) {
			...
			if (!dynamic_cast<VolumeConstPtr >(oldMat))
			...
			if (camera)
				camera->UpdateVolumeReferences(static_cast<VolumeConstPtr >(oldMat), static_cast<VolumeConstPtr >(newMat));
			// Check also the light source volumes
			lightDefs.UpdateVolumeReferences(static_cast<VolumeConstPtr >(oldMat), static_cast<VolumeConstPtr >(newMat));

			// Check also the world default volume
			if (defaultWorldVolume == oldMat)
				defaultWorldVolume = static_cast<VolumeConstPtr >(newMat);
...
Volume *Scene::CreateVolume(const u_int defaultVolID, const string &volName, const Properties &props) {
	...
	TextureConstPtr iorTex = GetTexture(props.Get(Property(propName + ".ior")(1.f)));
	...
	Volume *vol;
	if (volType == "clear") {
		TextureConstPtr absorption = GetTexture(props.Get(Property(propName + ".absorption")(0.f, 0.f, 0.f)));

		vol = new ClearVolume(iorTex, emissionTex, absorption);
	} else if (volType == "homogeneous") {
		TextureConstPtr absorption = GetTexture(props.Get(Property(propName + ".absorption")(0.f, 0.f, 0.f)));
		TextureConstPtr scattering = GetTexture(props.Get(Property(propName + ".scattering")(0.f, 0.f, 0.f)));
		TextureConstPtr asymmetry = GetTexture(props.Get(Property(propName + ".asymmetry")(0.f, 0.f, 0.f)));
