...
{
	// Nota: 'normals' are vertex normals. They are optional: if missing,
...
std::optional<std::vector<Normal>> ExtTriangleMesh::ComputeNormals() {
	bool allocated;
	if (not normals.has_value()) {
		allocated = true;
		normals.emplace(vertCount);
	} else
		allocated = false;
	...
	return allocated ? normals : std::nullopt;
}
...
ExtTriangleMeshPtr ExtTriangleMesh::CopyExt(
	...
	auto vs = meshVertices;
	if (!vs) {
		vs = AllocVerticesBuffer(vertCount);  // Cannot be emplaced, because of ending checksum
	...
	auto ns = meshNormals;
	if (!ns && HasNormals()) {
		ns.emplace(vertCount);
		std::copy(std::execution::par, normals->begin(), normals->end(), ns->begin());
	}
...
ExtTriangleMeshPtr ExtTriangleMesh::Merge(
	...
	auto meshVertices = AllocVerticesBuffer(totalVertexCount);
	auto meshTris = AllocTrianglesBuffer(totalTriangleCount);
	...
	if (meshes[0]->HasNormals())
		meshNormals.emplace(totalVertexCount);
